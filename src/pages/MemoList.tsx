...
import { Eye, Printer, Trash2, Archive, AlertTriangle, CheckCircle2, FileText, History } from "lucide-react";
...
export default function MemoList() {
  ...
  // Read active tab from URL, fallback to active
  const urlTab = searchParams.get("tab") || "active";
  const activeTab = urlTab === "archived" ? "archived" : "active";
    ...
    <div className="space-y-6 animate-fade-in pb-12">
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
        <TabsList className="mb-4 h-auto p-1 bg-secondary/20 border">
          <TabsTrigger value="active" className="data-[state=active]:bg-background data-[state=active]:shadow-sm px-4 py-2 gap-2">
            <FileText className="h-4 w-4" />
            Active Memos
          </TabsTrigger>
          <TabsTrigger value="archived" className="data-[state=active]:bg-background data-[state=active]:shadow-sm px-4 py-2 gap-2">
        ...
        <TabsContent value="archived" className="mt-0 outline-none">
          <ArchivesComponent embedded={true} />
        </TabsContent>
      </Tabs>
    </div>
  );
