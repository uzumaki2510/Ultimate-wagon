...
export function WorkflowChecklist({ wagon }: { wagon: Wagon }) {
  ...
  return <section className="space-y-5" aria-label="Wagon workflow">
    <div><h3 className="text-lg font-semibold">Work process</h3><p className="text-sm text-muted-foreground mt-1">{definition.name} · {resolved.completedCount} / {resolved.totalCount} recorded complete</p></div>
    {error && <p role="alert" className="rounded-lg border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive">{error}</p>}
    {notice && <p role="status" className="rounded-lg border p-3 text-sm">{notice}</p>}
    {locked ? <div className="rounded-xl border p-4 space-y-2"><h4 className="font-semibold">Workflow is read-only</h4><p className="text-sm text-muted-foreground">This wagon is certified or released. An administrator must reopen it before work or corrections can be saved.</p>{!allDone && <p className="text-sm text-amber-800 dark:text-amber-200">Saved workflow evidence is incomplete. Review the record; do not assume pending stages were completed.</p>}</div>
    : needsMigration ? <div role="alert" className="rounded-xl border p-4 text-sm">This record uses older stage names. Administrator review and a verified data migration are required before continuing. Existing history has been preserved.</div>
    ...
    : <div className="rounded-xl border-2 border-primary bg-primary/5 p-4 sm:p-5 space-y-4">
      ...
      <p className="text-xs text-muted-foreground">Saved under your signed-in account: {actor}.</p>
    ...
    <WorkRecordActions wagon={wagon} /><WorkAssignment wagon={wagon} /><FitnessConfirmation wagon={wagon} />
    <details className="rounded-xl border p-4"><summary className="cursor-pointer font-semibold py-1">Stage history & route</summary><div className="pt-4"><DefectTimeline wagon={wagon} /></div>
      {isAdmin && !locked && !needsMigration && workflow?.stages.filter(item => item.status === 'Done').map(item => <Button key={item.stageName} variant="outline" className="mt-3 w-full justify-start whitespace-normal h-auto min-h-11 text-left" onClick={() => { setCorrecting(item.stageName); setStaff(item.staffName || item.inspectorName || actor); setCorrectionRemarks(item.remarks || ''); }}>Correct {definition.stages[item.stageName]?.label || item.stageName}</Button>)}
    </details>
